...
def test_bucket_policy_list_bucket_with_prefix():
    ...
    policy = '''{
      "Version":"2012-10-17",
      "Statement": [{
        "Sid": "Allow Public listing of objects with folder prefix",
        "Effect": "Allow",
        "Principal": "*",
        "Action": "s3:ListBucket",
        "Condition": {
...
def test_bucket_policy_list_bucket_with_maxkeys():
    bucket = _create_keys(keys=['key'+str(i) for i in range(1,21)])
    policy = '''{
      "Version":"2012-10-17",
      "Statement": [{
        "Sid": "Allow Public listing of objects with folder prefix",
    ...
    res = new_conn.make_request('GET', bucket.name, query_args = 'max-keys=10')
    eq(res.status, 200)
    ns = {"aws" : "http://s3.amazonaws.com/doc/2006-03-01/"}
    keys = ET.fromstring(res.read()).findall('.//aws:Key', ns)
    eq(len(keys), 10)

    res = new_conn.make_request('GET', bucket.name, query_args = 'max-keys=9')
    eq(res.status, 200)
    ns = {"aws" : "http://s3.amazonaws.com/doc/2006-03-01/"}
    keys = ET.fromstring(res.read()).findall('.//aws:Key', ns)
    eq(len(keys), 9)

    res = new_conn.make_request('GET', bucket.name, query_args = 'max-keys=11')
    eq(res.status, 403)
...
def test_bucket_policy_list_bucket_with_delimiter():
    bucket = _create_keys(keys=['key/'+str(i) for i in range(1,21)])
    make_delim_policy = lambda(bucket_name): '''{
      "Version":"2012-10-17",
      "Statement": [{
        "Sid": "Allow Public listing of objects with folder prefix",
        "Effect": "Allow",
    ...
    policy = make_delim_policy(bucket.name)
    eq(bucket.set_policy(policy), True)
    ...
    bucket2 = _create_keys(keys=['key'+str(i) for i in range(1,21)])
    policy2 = make_delim_policy(bucket2.name)
