...
def put_role_policy(iam_client, rolename, policyname, role_policy):
    ...
    try:
        role_response = iam_client.put_role_policy(
            RoleName=rolename, PolicyName=policyname, PolicyDocument=role_policy
        )
    except ClientError as e:
        role_err = e.response["Code"]
...
def put_user_policy(iam_client, username, policyname, policy_document):
    ...
    try:
        role_response = iam_client.put_user_policy(
            UserName=username, PolicyName=policyname, PolicyDocument=policy_document
        )
    except ClientError as e:
        role_err = e.response["Code"]
...
def test_get_session_token():
    ...
    (resp_err, resp, policy_name) = put_user_policy(
        iam_client, sts_user_id, None, user_policy
    )
...
def test_assume_role_allow():
    ...
    policy_document = (
        '{"Version":"2012-10-17","Statement":[{"Effect":"Allow","Principal":{"AWS":["arn:aws:iam:::user/'
        + sts_user_id
    ...
    if role_response:
        assert (
            ...
            == "arn:aws:iam:::role/" + general_role_name + ""
        )
    else:
    ...
    (role_err, response) = put_role_policy(
        iam_client, general_role_name, None, role_policy
    )
    if response:
        assert response["ResponseMetadata"]["HTTPStatusCode"] == 200
    else:
    ...
    resp = sts_client.assume_role(
        RoleArn=role_response["Role"]["Arn"], RoleSessionName=role_session_name
    )
    assert resp["ResponseMetadata"]["HTTPStatusCode"] == 200
...
def test_assume_role_with_web_identity():
    ...
    policy_document = (
        ...
        + oidc_response["OpenIDConnectProviderArn"]
        + '"]},"Action":["sts:AssumeRoleWithWebIdentity"],"Condition":{"StringEquals":{"localhost:8080/auth/realms/'
        + realm
        + ':app_id":"'
        + aud
        + '"}}}]}'
    ...
    assert (
        role_response["Role"]["Arn"] == "arn:aws:iam:::role/" + general_role_name + ""
    )
...
def test_session_policy_check_on_different_buckets():
    ...
    if oidc_error is not None:
        raise RuntimeError(
            "Unable to create/get openid connect provider {}".format(oidc_error)
        )
    ...
    (role_err, response) = put_role_policy(
        iam_client, general_role_name, None, role_policy_new
    )
    assert response["ResponseMetadata"]["HTTPStatusCode"] == 200
    ...
    try:
        s3_put_obj = s3_client.put_object(
            Body=bucket_body, Bucket=bucket_name_1, Key="test-1.txt"
        )
    except ClientError as e:
...
def test_session_policy_bucket_policy_session_arn():
    ...
    resource2 = "arn:aws:s3:::" + bucket_name_1 + "/*"
    rolesessionarn = (
        "arn:aws:iam:::assumed-role/" + general_role_name + "/" + role_session_name
    )
    bucket_policy = json.dumps(
        {
            "Version": "2012-10-17",
...
def test_session_policy_copy_object():
    ...
    copy_source = {"Bucket": bucket_name_1, "Key": "test-1.txt"}
...
def test_assume_role_with_web_identity_with_request_tag():
    ...
    policy_document = (
        '{"Version":"2012-10-17","Statement":[{"Effect":"Allow","Principal":{"Federated":["'
        + oidc_response["OpenIDConnectProviderArn"]
        + '"]},"Action":["sts:AssumeRoleWithWebIdentity","sts:TagSession"],"Condition":{"StringEquals":{"aws:RequestTag/Department":"Engineering"}}}]}'
    )
    (role_error, role_response, general_role_name) = create_role(
        iam_client, "/", None, policy_document, None, None, None
    )
...
def test_assume_role_with_web_identity_tag_keys_trust_policy():
    ...
    policy_document = (
        '{"Version":"2012-10-17","Statement":[{"Effect":"Allow","Principal":{"Federated":["'
        + oidc_response["OpenIDConnectProviderArn"]
        + '"]},"Action":["sts:AssumeRoleWithWebIdentity","sts:TagSession"],"Condition":{"StringEquals":{"aws:TagKeys":"Department"}}}]}'
    )
    (role_error, role_response, general_role_name) = create_role(
        iam_client, "/", None, policy_document, None, None, None
    )
...
def test_assume_role_with_web_identity_resource_tag():
    ...
    bucket_body = "this is a test file"
    s3_put_obj = s3_client.put_object(
        Body=bucket_body, Bucket=bucket_name, Key="test-1.txt"
    )
    assert s3_put_obj["ResponseMetadata"]["HTTPStatusCode"] == 200
...
def test_assume_role_with_web_identity_wrong_resource_tag_deny():
    ...
    bucket_tagging = s3_res_iam_creds.BucketTagging(bucket_name)
    Set_Tag = bucket_tagging.put(
        Tagging={"TagSet": [{"Key": "Department", "Value": "WrongResourcetag"}]}
    )
...
def test_assume_role_with_web_identity_resource_tag_copy_obj():
    ...
    # copy to same bucket
    copy_source = {"Bucket": bucket_name, "Key": "test-1.txt"}
...
def test_assume_role_with_web_identity_role_resource_tag():
    ...
    policy_document = (
        '{"Version":"2012-10-17","Statement":[{"Effect":"Allow","Principal":{"Federated":["'
        + oidc_response["OpenIDConnectProviderArn"]
        + '"]},"Action":["sts:AssumeRoleWithWebIdentity","sts:TagSession"],"Condition":{"StringEquals":{"iam:ResourceTag/Department":"Engineering"}}}]}'
    )
    tags_list = [
        {"Key": "Department", "Value": "Engineering"},
        {"Key": "Department", "Value": "Marketing"},
    ...
    (role_error, role_response, general_role_name) = create_role(
        iam_client, "/", None, policy_document, None, None, None, tags_list
    )
    assert (
        role_response["Role"]["Arn"] == "arn:aws:iam:::role/" + general_role_name + ""
    )
