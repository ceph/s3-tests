...
def test_session_policy_bucket_policy_role_arn():
    ...
    s3object_error = 'AccessGranted'
    try:
        s3_client.get_object(Bucket=bucket_name_1, Key="test-1.txt")
    except ClientError as e:
...
def test_assume_role_with_web_identity_resource_tag():
    ...
    s3_put_obj = s3_client.put_object(Body='this is a test file', Bucket=bucket_name, Key="test-1.txt")
    assert s3_put_obj['ResponseMetadata']['HTTPStatusCode'] == 200
...
def test_assume_role_with_web_identity_resource_tag_deny():
    ...
    s3_put_obj_error = 'AccessGranted'
    try:
        s3_client.put_object(Body='this is a test file', Bucket=bucket_name, Key="test-1.txt")
    except ClientError as e:
        s3_put_obj_error = e.response.get("Error", {}).get("Code")
    assert s3_put_obj_error == 'AccessDenied'
