...
def test_put_user_policy_parameter_limit():
    ...
    policy_document = json.dumps(
        {
            "Version": "2012-10-17",
            "Statement": [{"Effect": "Allow", "Action": "*", "Resource": "*"}] * 1000,
        }
    )
    e = assert_raises(
        ClientError,
...
def test_list_user_policy_invalid_user():
    client = get_iam_client()
    e = assert_raises(
        ClientError, client.list_user_policies, UserName="some-non-existing-user-id"
    )
    status = _get_status(e.response)
    assert status == 404
...
def test_get_user_policy_invalid_policy_name():
    ...
    e = assert_raises(
        ClientError,
        client.get_user_policy,
        PolicyName="non-existing-policy-name",
        UserName=get_alt_user_id(),
    )
    status = _get_status(e.response)
    assert status == 404
...
def test_get_user_policy_from_multiple_policies():
    ...
    response = client.put_user_policy(
        PolicyDocument=policy_document_allow,
    ...
    assert response["ResponseMetadata"]["HTTPStatusCode"] == 200
    response = client.get_user_policy(
        PolicyName="AllowAccessPolicy2", UserName=get_alt_user_id()
    )
    assert response["ResponseMetadata"]["HTTPStatusCode"] == 200

    response = client.delete_user_policy(
        PolicyName="AllowAccessPolicy1", UserName=get_alt_user_id()
    )
    assert response["ResponseMetadata"]["HTTPStatusCode"] == 200
    response = client.delete_user_policy(
        PolicyName="AllowAccessPolicy2", UserName=get_alt_user_id()
...
def test_delete_user_policy_from_multiple_policies():
    ...
    assert response["ResponseMetadata"]["HTTPStatusCode"] == 200
    response = client.get_user_policy(
        PolicyName="AllowAccessPolicy3", UserName=get_alt_user_id()
    )
    assert response["ResponseMetadata"]["HTTPStatusCode"] == 200
...
def test_deny_bucket_actions_in_user_policy():
    ...
    response = client.put_user_policy(
        PolicyDocument=policy_document_deny,
...
def test_deny_object_actions_in_user_policy():
    ...
    assert error_code == "AccessDenied"
    e = assert_raises(
        ClientError, s3_client_alt.delete_object, Bucket=bucket, Key="foo"
    )
    status, error_code = _get_status_and_error_code(e.response)
    assert status == 403
    assert error_code == "AccessDenied"
...
def test_allow_multipart_actions_in_user_policy():
    ...
    (upload_id, _, _) = _multipart_upload(
        client=s3_client_iam, bucket_name=bucket, key=key, size=5 * mb
    )
    response = s3_client_alt.list_multipart_uploads(Bucket=bucket)
    assert response["ResponseMetadata"]["HTTPStatusCode"] == 200
    response = s3_client_alt.abort_multipart_upload(
...
def test_deny_multipart_actions_in_user_policy():
    ...
    (upload_id, _, _) = _multipart_upload(
        client=s3_client, bucket_name=bucket, key=key, size=5 * mb
    )
...
def test_allow_tagging_actions_in_user_policy():
    ...
    assert response["ResponseMetadata"]["HTTPStatusCode"] == 200
    response = s3_client_alt.put_object_tagging(
        Bucket=bucket, Key=obj_key, Tagging=tags
    )
    assert response["ResponseMetadata"]["HTTPStatusCode"] == 200
...
def test_deny_tagging_actions_in_user_policy():
    ...
    e = assert_raises(
        ClientError, s3_client.put_bucket_tagging, Bucket=bucket, Tagging=tags
    )
    status, error_code = _get_status_and_error_code(e.response)
    assert status == 403
    assert error_code == "AccessDenied"
    ...
    assert error_code == "AccessDenied"
    e = assert_raises(
        ClientError, s3_client.delete_object_tagging, Bucket=bucket, Key=obj_key
    )
    status, error_code = _get_status_and_error_code(e.response)
    assert status == 403
    assert error_code == "AccessDenied"
...
def test_verify_conflicting_user_policies():
    ...
    client = get_iam_client()
    response = client.put_user_policy(
        PolicyDocument=policy_allow,
        PolicyName="AllowAccessPolicy",
        UserName=get_alt_user_id(),
    )
    assert response["ResponseMetadata"]["HTTPStatusCode"] == 200
    response = client.put_user_policy(
        PolicyDocument=policy_deny,
        PolicyName="DenyAccessPolicy",
        UserName=get_alt_user_id(),
    )
    assert response["ResponseMetadata"]["HTTPStatusCode"] == 200
...
def test_verify_allow_iam_actions():
    ...
    response = client1.put_user_policy(
        PolicyDocument=policy1,
        PolicyName="AllowAccessPolicy",
        UserName=get_alt_user_id(),
    )
    assert response["ResponseMetadata"]["HTTPStatusCode"] == 200
    response = iam_client_alt.get_user_policy(
        PolicyName="AllowAccessPolicy", UserName=get_alt_user_id()
    )
    assert response["ResponseMetadata"]["HTTPStatusCode"] == 200
    response = iam_client_alt.list_user_policies(UserName=get_alt_user_id())
    assert response["ResponseMetadata"]["HTTPStatusCode"] == 200
    response = iam_client_alt.delete_user_policy(
        PolicyName="AllowAccessPolicy", UserName=get_alt_user_id()
    )
    assert response["ResponseMetadata"]["HTTPStatusCode"] == 200
